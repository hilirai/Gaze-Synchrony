...
# ====== CONFIG ======
CHECKPOINT  = os.environ.get("SAM2_CHECKPOINT", "/opt/sam2/checkpoints/sam2.1_hiera_base_plus.pt")
MODEL_CFG   = os.environ.get("SAM2_MODEL_CFG", "/opt/sam2/server/sam2/configs/sam2.1/sam2.1_hiera_b+.yaml")
VIDEO_DIR   = os.environ.get("VIDEO_DIR", "/app/workspace/video_parts")
OUTPUT_DIR  = os.environ.get("OUTPUT_DIR", "/app/workspace/output")
DEVICE      = "cuda" if torch.cuda.is_available() else "cpu"
GAZE_RADIUS = 50  # pixels

# Shared with Flask: where it serves /static from (env overrides)
SCRIPT_DIR  = os.path.dirname(os.path.abspath(__file__))
SCRIBBLE_STATIC_DIR = os.environ.get("SCRIBBLE_STATIC_DIR", "/app/static")
...
def main():
    ...
    # Save into both OUTPUT_DIR and Flask static folder
    grid_path = os.path.join(OUTPUT_DIR, "seed_grid.jpg")
    cv2.imwrite(grid_path, frame)
    static_grid = os.path.join(SCRIBBLE_STATIC_DIR, "seed_grid.jpg")
    cv2.imwrite(static_grid, frame)
    print(f"[info] Wrote grid to: {grid_path} and {static_grid}")
